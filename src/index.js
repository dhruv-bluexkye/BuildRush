...
window.TowerGame = (option = {}) => {
  ...
  // Get timer duration from options (default to 15 seconds)
  const timerDuration = option.timerDuration || 15
  game.setVariable(constant.gameTimer, timerDuration)
  game.setVariable(constant.gameTimerStartTime, null)
  for (let i = 1; i <= 4; i += 1) {
  ...
  game.start = () => {
    ...
    // Initialize timer start time when game starts
    game.setVariable(constant.gameTimerStartTime, Date.now())
  }
