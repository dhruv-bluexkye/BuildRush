...
export const endAnimate = (engine) => {
  ...
  const timerStartTime = engine.getVariable(constant.gameTimerStartTime)
  const gameUserOption = engine.getVariable(constant.gameUserOption)
  const timerDuration = (gameUserOption && gameUserOption.timerDuration) || 15
  let gameTimer = engine.getVariable(constant.gameTimer, timerDuration)
  if (timerStartTime !== null) {
    const elapsed = (Date.now() - timerStartTime) / 1000 // elapsed time in seconds
    const remaining = Math.max(0, timerDuration - elapsed) // Use timer duration from options
    gameTimer = Math.ceil(remaining)
    engine.setVariable(constant.gameTimer, gameTimer)
    
    // Trigger game over when timer reaches 0
    if (gameTimer <= 0 && gameStartNow) {
      ...
      if (setGameFailed) {
        setGameFailed(3) // Force game over by setting failed count to 3
      }
    }
  }
